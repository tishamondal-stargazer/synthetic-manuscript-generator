...
def make_paper(seed):
    ...
    for _ in range(5):
        ...
        draw.line(
            (45, y, WIDTH - 45, y + rng.randint(-3, 3)),
            fill=(100, 70, 40, 28),
...
def render_folio(script, config, passage, index, split, seed):
    ...
    # Use a font included in the project for portable headings.
    heading_font = ImageFont.truetype(
        str(PROJECT_DIR / "fonts" / "NotoSansDevanagari-Regular.ttf"),
        24,
    )
        ...
        for split, count in split_plan.items():
            ...
            for local_index in range(count):
                ...
                global_index = (
                    sum(
                        split_plan[s]
                        for s in list(split_plan)[:list(split_plan).index(split)]
                    )
                    + local_index
                )
